...
export default function SiteHeader() {
  ...
  const [indicator, setIndicator] = useState<{ left: number; width: number; opacity: number; instant: boolean }>({
    left: 0,
    width: 0,
    opacity: 0,
    instant: true,
  ...
  /* 홈(/) 진입 시 로고 골드 순차 점등 → 잠시 후 원래 색으로 복귀(일시 강조). */
  const [logoGold, setLogoGold] = useState(false);
  const goldTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const lightLogoGold = useCallback(() => {
    setLogoGold(true);
    if (goldTimerRef.current) clearTimeout(goldTimerRef.current);
    /* 점등 완료(stagger 585 + fill 360 ≈ 945ms) 후 잠시 유지하다 복귀 */
    goldTimerRef.current = setTimeout(() => setLogoGold(false), 1150);
  }, []);
  ...
  const measureIndicator = useCallback((instant: boolean) => {
    ...
    if (!active) {
      setIndicator((prev) => ({ ...prev, width: 0, opacity: 0, instant }));
      return;
    }
    /* 이전이 숨김(width 0 — 홈/비행 후)이면 instant 로 active 자리에 바로 표시
       (좌측에서 날아오는 사이드이펙트 차단). 메뉴↔메뉴(prev.width>0)만 sliding. */
    setIndicator((prev) => ({
      left: active.offsetLeft,
      width: active.offsetWidth,
      opacity: 1,
      instant: instant || prev.width === 0,
    }));
  }, []);
    ...
    letters.forEach((el, i) => {
      ...
      el.animate(
        [
          ...
          { transform: `translateY(${peak2}px)`, offset: 0.78 },
          { transform: 'translateY(0)' },
        ],
  ...
  const triggerLogoIntro = useCallback(() => {
    setIndicator((prev) => ({ ...prev, width: 0, opacity: 0 }));
    bounceLogoLetters();
    lightLogoGold();
  }, [bounceLogoLetters, lightLogoGold]);

  /* pathname 변경 → 인디케이터 갱신. 첫 측정만 instant(점프 없이 위치 확정), 이후
     라우트 변경은 transition sliding. 홈은 위 flyToLogoAndBounce 로 분기.
     paint 전(useLayoutEffect) 측정으로 깜빡임 차단. */
  useLayoutEffect(() => {
    const firstMount = !indicatorReadyRef.current;
    indicatorReadyRef.current = true;
    if (pathname === '/') {
      if (firstMount) {
        /* 홈 직접 로드 — bounce 없이 골드만 점등 후 복귀 */
        setIndicator((prev) => ({ ...prev, width: 0, opacity: 0, instant: true }));
        lightLogoGold();
      } else {
        /* 클릭 진입 — 로고 bounce + 순차 골드 점등 + 복귀 */
        triggerLogoIntro();
      }
      return;
    }
        ...
        <div className="hdr">
          ...
          <div className="hdr-left">
            ...
            >
              <svg
                id="logo-img"
                className={logoGold ? 'is-home' : undefined}
              ...
              >
                {/* 글자별 <g> — x좌표 순(good g·o·o·d / things t·h·i·n·g·s). 충돌점=good g(첫째).
                    transform 없음 → 시각 무변화. .lg-letter 가 bounce 애니메이션 대상. */}
                {/* good */}
                <g className="lg-letter">
