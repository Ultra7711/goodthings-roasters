/* ══════════════════════════════════════════
   ...
   - 수량: stepper (변경 없음)
   - 정기배송: 탭 → 체크박스 격하 ("정기배송으로 받기" + 체크 시 주기 select reveal)
...
export default function PurchaseRow({ product, volIdx, onVolChange }: Props) {
  ...
  /* CTA 우측 가격 = 단가 × 수량 (정기배송 할인은 D-15 carry-over) */
  const unitPrice = hasVolumes ? product.volumes[volIdx].price : 0;
  const totalPrice = unitPrice * qty;
  const totalPriceStr = `${totalPrice.toLocaleString('ko-KR')}원`;
  ...
  return (
    <div className={`pd-purchase${disabled ? ' pd-purchase--disabled' : ''}`}>
      {/* 용량 / 품목 chip 그룹 */}
      {hasVolumes && (
      ...
      {/* 정기배송 박스 — 체크박스 + 주기 트리거가 한 컨테이너에 묶여 부모-자식 관계 명확.
          체크 전 우측은 비움 (마케팅 문구는 어드민 결정 후) — D-17 carry-over. */}
      {canSubscribe && (
        <div id="pd-subscription-block">
          ...
          >
            <label className="pd-sub-toggle">
              ...
              <span className="pd-sub-toggle-label">정기배송으로 받기</span>
            </label>
      ...
      {/* 수량 스텝퍼 + 장바구니 한 줄 — 최종 카트 담기 직전 수량 결정 UX */}
      <div id="pd-cta-row">
        <div id="pd-qty-block">
          <div className="pd-input-box pd-qty-box">
        ...
        >
          ...
          {!disabled && (
            <>
              ...
              <span className="pd-cart-btn-price">{totalPriceStr}</span>
            </>
          )}
        </div>
      </div>
