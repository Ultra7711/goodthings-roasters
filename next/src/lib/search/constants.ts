...
export const SYNONYM_CLASSES: readonly (readonly string[])[] = [
  ...
  // 에스프레소 음료
  ['latte', '라떼', '라테'],
  // 아메리카노 클래스 — 줄임말(아아/따아) 통합. 소비자 구어가 실제 메뉴(b04
  // "아메리카노")로 매칭되도록 단일 equivalence class 로 묶음. DB 에 별도
  // "아이스아메리카노" 항목은 없지만 양방향 동의어로 미래 데이터 확장 대응.
  ['americano', '아메리카노', '아아', '아이스아메리카노', '따아', '따뜻한아메리카노'],
