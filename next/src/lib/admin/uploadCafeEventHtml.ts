/* ══════════════════════════════════════════════════════════════════════════
   uploadCafeEventHtml.ts — cafe_events 운영자 HTML 파일 업로드 (S234 후속 · 060)

   책임:
   - .html 파일 사이즈/MIME 사전 검증 (UX — 실제 enforcement 는 RLS)
   - cafe-events 버킷 + prefix 'html/' (060 마이그)
   - public URL 반환 (custom_html_path 로 cafe_events 행에 저장)

   설계:
   - uploadCafeEventImage / uploadCafeEventCss 패턴 답습.
   - 060 마이그가 cafe-events 버킷의 allowed_mime_types 에 text/html 추가.
   - EventBanner 가 <iframe sandbox="allow-same-origin"> 으로 임베드 →
...
export async function uploadCafeEventHtml(file: File): Promise<UploadResult> {
  /* 1) 사전 검증 — 크기 + MIME + 확장자 */
