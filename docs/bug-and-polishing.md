...
> **68 / 71 closure (95.8%)** · 2026-04-28 S94 기준 (S93: BUG-174/175 ✅ · S94: BUG-137 ✅ BUG-160 동일원인 · BUG-176 ✅ 신규+closure · BUG-101 제거 · BUG-173 미해결)
>
> 카운트 명령:
> ```bash
...
- **PR1 완료 (2026-04-27 / S86):**
  - 마이그레이션 023 (enum 분리 — `payment_method += 'easypay'` + `easypay_provider` 신규 9종 enum) + 024 (orders/payments 컬럼 + CHECK 교체 + confirm_payment RPC 확장 + `get_easypay_provider_breakdown` RPC).
  - `paymentService.ts` TOSS_METHOD_TABLE 확장 (`간편결제`/`EASY_PAY`) + `TOSS_PROVIDER_TABLE` 신규 (9종 한/영) + `mapEasyPayProvider` + `method !== order.payment_method` 비교 검증 제거 (webhook 권위 원칙) + 새 에러 `easypay_provider_missing`.
  - `paymentRepo.ConfirmPaymentRpcParams` += `easypayProvider`, SELECT 에 `easypay_provider` 포함.
