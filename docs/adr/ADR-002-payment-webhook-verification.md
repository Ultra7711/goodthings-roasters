...
| **IP 화이트리스트 *보완* 방어** (security H-3, Session 6 제안) | **2026-04-17 기각.** 이유: (1) Toss 가 공식 IP 를 문서에 공개하지 않으며 고객센터 회신의 정확성·변경 공지가 불투명. (2) 본 ADR 의 하이브리드 인증(카드=GET 권위 재조회, 가상계좌=timing-safe secret)이 Stripe 의 서명 검증과 동등한 역할 — OWASP Webhook Security Cheat Sheet 의 "서명이 있으면 IP 는 중복" 원칙에 부합. (3) 국내 커머스 레퍼런스 리서치에서도 IP allowlist 단독 의존 사례는 발견되지 않음. 회신 대기 중 관측 로깅(payment_events.raw_payload 의 x-forwarded-for)만 유지. 상세: `docs/security-research-2026-04-16.md §2.3`. |
