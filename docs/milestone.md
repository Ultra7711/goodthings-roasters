...
| 3 (§2.2) | 메인 시그니처 chapter PR-1b — 화살표 제거 + active translateY → bg darken (Blue Bottle 패턴) + 모바일 stacking fix (사진 위 / 텍스트 아래) + sticky hover 차단 | ✅ S147 (`6a7d0b72`) |
| 3 (§2.2) | 시그니처 chapter PR-1c — 전 사이트 CTA 33+종 Blue Bottle 패턴 통일 (sig + 메인 + PDP + 인증 + 주문 + 마이페이지 + 비즈니스/시스템 + Kakao map · 7 카테고리 분리 commit). Gold underline ::after 시스템 전체 폐기. 토큰 재정의: `--color-btn-primary-bg #1C → #36332E warm textured` / `-hover/-active #000` (Blue Bottle 매치). secondary CTA 신규 (`.cta-btn-secondary` + chp-empty-cta · mp-cancel-btn · mp-modal-cancel · cta-btn-light-outline · st-map-overlay-btn 마이그레이션). 토큰 신규 (`--color-background-pure: #FFFFFF` · `--color-btn-light-bg-active`). option-chip-selected 토큰 정합 | ✅ S147 (`c1081e63` + `f0acc19b` + `05558124` + `cefcb9c5` + `c66afaaa` + `04134f2c`) |
| 3 (§2.2) | 시그니처 chapter PR-2 — advisory §6 어드민 워크플로 (SettingsForm 시그니처 섹션 카드 7항목 입력: enabled · eyebrow + 분기 자동 채움 SS/SU/FW/WT · Coffee Bean 드롭다운 · title · subtitle 80자 hint · flavor_chips Tasting Notes 자동 + Enter/× 추가삭제 · 5:4 image + 안전 영역 가이드 inset 12% 18% · alt) + **라이브 미리보기** (`/preview/signature` route 신설 · admin 가드 · URL 파라미터 → SignatureSettings Zod safeParse · SignatureChapter → View 분리 · 빈 상태 placeholder · 4 brk iframe 토글 1440/1024/768/360 · 300ms debounce · postMessage height 동기 ResizeObserver) + CSP frame-ancestors 분기 (proxy.ts: `/` + `/preview/*` → 'self' / 그 외 'none' OWASP 부합) + season-banners 버킷 재사용 (signature/ prefix) + actions.ts SaveInputSchema signature 추가. 분기 캘린더 D-7 알림은 후속 sprint (D-2 결정) | ✅ S148 (`d7b38a12`) |
