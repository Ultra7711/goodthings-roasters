...
src/components/admin/...             — admin 공통 컴포넌트
  ...
  📋 (Sprint 1-B 추출 예정 · S227)
  📋   AdminPageHeader        — h2 + subtitle + 카운트 통일 (DEC-8)
  📋   AdminPagination        — 26×26 PageNav 표준 (DEC-9)
  📋   AdminListMeta          — "총 N건" 외부 푸터 (DEC-14)
  📋   AdminEmptyState        — 테이블 colSpan / 카드 안 표준
  📋   AdminTabsNav           — URL state / local state 양 모드 (DEC-11)
  📋   AdminDropdownFilter    — 활성 표시 통일 (Orders/Subscriptions 답습)
  📋   AdminDataTable         — shadcn Table wrap (DEC-10)
...
src/lib/admin/...                    — server fetcher + 매핑 헬퍼
  ...
  ❌ products / cafeEvents / settings 누락 (S230 Sprint 3 분리 예정)
```
...
## 12. 다음 Sprint 카탈로그 (S227~S231)

상세: `memory/project_admin_ui_unification_plan.md`

| Sprint | 범위 | 추정 | Agent / Skill |
|---|---|---|---|
| **S227 (Phase 1)** | Architecture audit + 표준 컴포넌트 8종 추출 + DEC-8~14 잠금 + design.md §13 박음 | 12~16h | `code-architect` + mattpocock `diagnose` / `improve-codebase-architecture` / `zoom-out` |
| **S228~S229 (Phase 2)** | 페이지별 적용 (테이블 / 상세 / 단일 — 3 PR) | 24~32h | `code-reviewer` + `tdd-guide` |
| **S230 (Phase 3)** | 잘못 구현 fix 일괄 — §7-3 hex Type 1+2 / ProductEditForm 탭 결정 / Topbar 정책 / lib 누락 3종 | 8~12h | `refactor-cleaner` + `type-design-analyzer` |
| **S231 (Phase 4)** | products/new 재기획 (실 PDP 모델 + RHF + create mode) | 12~16h | `planner` + `code-architect` + mattpocock `grill-with-docs` |
| **별 (Phase 5)** | improve-codebase-architecture 최종 refactor + ADR-009 | 8~10h | `code-architect` + `zoom-out` |

**총 5 sprints · 64~86h · 3~4주 작업 단위**

### Plan 잠금 (DEC-8~14)

| DEC | 내용 | Sprint |
|---|---|---|
| DEC-8 | 페이지 카운트 위치 = AdminPageHeader subtitle | 1-C |
| DEC-9 | 페이지네이션 위치 = 테이블 카드 안 하단 | 1-C |
| DEC-10 | 어드민 테이블 = AdminDataTable (TH/TD inline 폐기) | 1-C |
| DEC-11 | 어드민 탭 = AdminTabsNav | 1-C |
| DEC-12 | ← back 링크 = AdminBackLink | 1-C |
| DEC-13 | Topbar 미구현 carry-over = `disabled + tooltip` (mock 제거) | 2-B |
| DEC-14 | "총 N건" 라벨 = AdminListMeta (외부 푸터 · filtered.length 우선) | 2-A |
