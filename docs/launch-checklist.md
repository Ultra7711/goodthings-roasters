...
- [ ] ⬜ **Supabase 운영 DB 분리** (전략 확정·S341) — 코드는 env 교체만으로 분리 가능(하드코딩 0). 현재 로컬·Vercel 동일 프로젝트(미분리).
  **전략(DEC 후보): 임시 도메인을 dev로 강등 + 정식 도메인에 새 prod.**
  ...
  - **✅ auth(회원 계정) 포함 백업 확인** (S341 실측·정정): `db dump --data-only` 가 **auth.users(비번해시 포함)+auth.identities(OAuth) 데이터를 포함**한다(복원검증 실측: auth.users 10·identities 11·profiles 10 일치). → 현재 무료 백업으로 **주문·상품·설정 + 회원 계정까지 완전 복구 가능**. `roles` 만 미포함(supabase 관리·새 프로젝트 기본). 복원 시 `session_replication_role=replica` FK 우회. ※ 앞서 "auth 미백업" 기록은 오류였음(정정).
